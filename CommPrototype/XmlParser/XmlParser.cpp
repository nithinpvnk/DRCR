...
void XmlParser::showAttributes()
{
  ...
  for (size_t i = 0; i < attribs_.size(); ++i)
  {
    //std::cout << "\n      " << attribs_[i].first << ", " << attribs_[i].second;
	  //dependency = dependency + attribs_[i].first + attribs_[i].second + ",";
  }
}
...
std::string XmlParser::returnDependency()
{
	return dependency;
}
...
int main()
{
  Repo::title("Testing XmlParser",'=');
  Repo::printLine();
  std::string src = "../XmlElementParts/metadata.xml";
  XmlParser parser(src);
  parser.verbose();
  Repo::title("Compressed form of formatted XML:");
  std::cout << "\n" << src << "\n";
  Repo::title("Parsing compressed XML:");
  XmlDocument* pDoc = parser.buildDocument();
  Repo::title("Resulting XML Parse Tree:");
  std::cout << "\n" << pDoc->toString();
  std::cout << "\n\n";
}
